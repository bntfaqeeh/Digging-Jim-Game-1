...
AnimatedImage.prototype.move = function (
  duration,
  dx,
  dy,
  keys,
  ...
  if (keys != null && keys != undefined && keys.length > 0) {
    if (keys.length == 1) this.setSrc(keys[0]);
    else this.srcAnime.play(keys, duration);
  }
...
AnimatedImage.prototype.moveX = function (duration, dx, keys, onFinish) {
  if (keys != null && keys != undefined && keys.length > 0) {
    console.log("Setting sprite for X movement:", keys);
    if (keys.length == 1) this.setSrc(keys[0]);
    else this.srcAnime.play(keys, duration);
  }
...
AnimateMove.prototype.playX = function (dx, durationX, onFinish) {
  ...
  if (!this.isPlayX) {
    ...
    //this.onFinish = onFinish;
    var self = this;
    this.moveTimerX = setInterval(
      function () {
        if (
          self.speedX > 0 &&
          self.image.getLeft() < self.targetX - self.speedX
        ) {
          ...
          self.isPlayX = false;
          if (onFinish != undefined && onFinish != null) onFinish();
          // else
...
AnimateMove.prototype.playY = function (dy, durationY, onFinish) {
  if (!this.isPlayY) {
    ...
    //console.log(this);
    var self = this;
    this.moveTimerY = setInterval(
      function () {
        if (
          self.speedY > 0 &&
          self.image.getTop() < self.targetY - self.speedY
        ) {
